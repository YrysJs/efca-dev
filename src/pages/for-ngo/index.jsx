...
const AnnualReports = () => {
  ...
  const enableSearch = (query) => {
    router.push({ pathname: '/for-ngo', query: removeEmpty({ ...router.query, ...query })})
  }
  ...
  return (
    <>
      <Head>
        <title>{t('materials.head.ngo')}</title>
      </Head>
      <section className="pt-6 px-3 md:pt-8 md:px-8 lg:pt-6">
        <Container>
          <div className="w-full flex items-center justify-between">
            <h1 className="text-2xl/8 lg:text-3xl font-bold text-primaryDark uppercase">{t('materials.head.ngo')}</h1>
...
export async function getServerSideProps(context) {
  ...
  const response = await api.get(`/materials?page=${query.page || 1}&type=ngo`, {
    params: query,
