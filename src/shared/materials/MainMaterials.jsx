...
const PrevSlideBtn = props => {
  return (
    <button {...props} className="absolute left-[35px] bottom-[-30px] mr-4 outline-none z-20">
      ...
      >
        <path
          d="M1.12102 8.22664C0.730494 7.83611 0.730494 7.20295 1.12102 6.81242L7.48498 0.448463C7.8755 0.0579391 8.50867 0.0579391 8.89919 0.448463C9.28972 0.838988 9.28972 1.47215 8.89919 1.86268L3.24234 7.51953L8.89919 13.1764C9.28972 13.5669 9.28972 14.2001 8.89919 14.5906C8.50867 14.9811 7.8755 14.9811 7.48498 14.5906L1.12102 8.22664ZM19.8281 8.51953L1.82812 8.51953V6.51953L19.8281 6.51953V8.51953Z"
          fill="#0006BB"
        />
      </svg>
    </button>
...
const NextSlideBtn = props => {
  return (
    <button {...props} className="absolute bottom-[-30px] right-[35px] ml-4 outline-none z-20">
      ...
      >
        <path
          d="M19.5352 8.22664C19.9258 7.83611 19.9258 7.20295 19.5352 6.81242L13.1713 0.448463C12.7807 0.0579391 12.1476 0.0579391 11.7571 0.448463C11.3665 0.838988 11.3665 1.47215 11.7571 1.86268L17.4139 7.51953L11.7571 13.1764C11.3665 13.5669 11.3665 14.2001 11.7571 14.5906C12.1476 14.9811 12.7807 14.9811 13.1713 14.5906L19.5352 8.22664ZM0.828125 8.51953L18.8281 8.51953V6.51953L0.828125 6.51953L0.828125 8.51953Z"
          fill="#0006BB"
        />
      </svg>
    </button>
...
const MainMaterials = ({data, translate, route}) => {
  ...
  const enableSearch = query => {
    router.push({ pathname: `/${route}`, query: removeEmpty({ ...router.query, ...query }) })
  }
            ...
            <div className="flex flex-end h-[38px]">
              <div className="ml-auto flex shadow pr-3 rounded-lg flex-row items-center justify-between w-[160px] md:w-[250px]">
                <input
                  ...
                  onKeyDown={e =>
                    e.key === 'Enter' ? enableSearch({ search: searchField || null }) : ''
                  }
                />
                <div
                  className="cursor-pointer"
                  onClick={() => enableSearch({ search: searchField || null })}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="21"
            ...
            {data.data.map((item, index) => (
              <Link href={`/${route}/${item.id}`} key={index} className="h-[576px] relative">
                ...
                <div className="absolute bottom-[24px] lg:bottom-[48px] left-[24px] lg:left-[48px] right-[24px] lg:right-[48px] text-white">
                  ...
                  <h3 className="text-2xl lg:text-[32px] font-bold my-4">{item.title}</h3>
                  <p className="text-base lg:text-base max-w-[520px]">{item.title}</p>
                </div>
              </Link>
            ))}
          </Slider>
